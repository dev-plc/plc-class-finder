...
* **식별 편의성**: 조원 이름 뒤에 번호가 붙어 표시됩니다 (예: `[이름]`).
...
1. **파일 확장자**: 파일명이 반드시 `.md`로 끝나야 합니다.
2. **공백**: `#` (제목) 뒤에는 반드시 **한 칸의 공백**이 있어야 합니다.
3. **줄바꿈**: 마크다운은 엔터를 한 번만 치면 줄바꿈이 안 될 수 있습니다. 문단 사이에는 **빈 줄을 하나 더** 넣어주세요.
4. **특수문자**: `*`나 `-` 같은 기호가 글자와 붙어 있으면 목록으로 인식되지 않으니 꼭 한 칸 띄워주세요.

이제 이 코드를 사용하시면 GitHub 등에서 아주 깔끔하게 보일 거예요! 추가로 디자인 수정이 필요한 부분이 있다면 말씀해주세요. :)
