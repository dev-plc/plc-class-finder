...
시트에 직접 입력 ┬→ pushAttendanceToDb (10분 트리거) → DB   ← 자가 치유 · 지우기
                └→ ⟳ · 정오 크론 → sync-sheet-to-db.mjs → DB  ← 즉시성
```
...
- **동기화도 출석을 가져온다 — 단 값이 있는 칸만.** 빈칸을 보내면 앱이 찍은 출석을
  덮어 지우므로 아예 행을 만들지 않는다 (DG 와 같은 방식).
  행이 없는 것과 `''` 인 행은 `coalesce(a.status,'')` 때문에 판정에서 완전히 같다
- **`--import-attendance` 는 '가져올까' 가 아니라 '빈칸까지 덮어쓸까' 다.**
  켜면 시트에서 비운 칸이 DB 에서도 지워지고 미래 주차 `O` 가 `◎` 로 들어간다 —
  **기수를 처음 만들 때만** 켠다
- **지우기는 10분 트리거만 할 수 있다.** DB 값과 비교해 차이만 올리므로
  '비웠다' 는 뜻을 담아 보낼 수 있다. 동기화와 트리거는 대체 관계가 아니다 —
  동기화는 즉시성, 트리거는 지우기와 자가 치유다. 트리거를 끄면 안 된다
...
```
O     출석
◎     지난 기수 이수 이월       ← 이월 스크립트와 기수 첫 구축 동기화만 찍는다
...
- **아직 하지 않은 주차의 값은 지우지 않는다.** 지난 기수에서 이어 듣는 사람에게
  그것은 "어디까지 들었고 무엇이 남았는지" 의 근거다 — `X` 는 지금 과제·소감문으로
  메울 수 있어야 하고, `과제` 는 보충 횟수로 세야 한다. `O` 만 `◎` 로 옮긴다
  (아직 하지 않은 수업에 출석은 있을 수 없다). `--import-attendance` 가 한때
  셋을 전부 빈칸으로 지웠는데, 그러면 그 근거가 같이 사라진다
