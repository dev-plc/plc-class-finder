...
import {
    ...
    subscribe,
    isTutorRole,
} from './scripts/members-data.js?v=109';
import { matches as hangulMatches } from './scripts/hangul.js?v=109';
import { registerServiceWorker } from './scripts/sw-update.js?v=109';
import { sbPostGas, sbSelect } from './scripts/supabase-config.js?v=109';
...
import {
    ...
    renderTeamMatrix,
    renderMatrixFold,
} from './scripts/attendance-matrix.js?v=109';
...
async function showLastSynced() {
    ...
    setSyncInfo(`마지막 동기화: ${agoText(at)}` +
        ...
        ' · 출석은 10분마다 저절로 들어옵니다. 방금 시트에서 고치셨다면'
        + ' ⟳ 를 눌러 주세요 (출석도 함께 가져옵니다).', stale ? 'fail' : '');
}
